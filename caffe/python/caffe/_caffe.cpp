...
BP_GET_POINTER(AdaBoundSolver, float);
#endif
...
void Net_SetInputArrays_with_name(Net<Dtype>* net, string layer_name, bp::object data_obj,
	bp::object labels_obj) {
	// check that this network has an input MemoryDataLayer
	int idx = net->find_id_by_name(layer_name);
	shared_ptr<MemoryDataLayer<Dtype> > md_layer =
		boost::dynamic_pointer_cast<MemoryDataLayer<Dtype> >(net->layers()[idx]);
	if (!md_layer) {
		throw std::runtime_error(layer_name + "can not be found");
	}
	bool has_label = md_layer->has_label();
	// check that we were passed appropriately-sized contiguous memory
	PyArrayObject* data_arr =
		reinterpret_cast<PyArrayObject*>(data_obj.ptr());
	CheckContiguousArray(data_arr, "data array", md_layer->channels(),
	...
	if (has_label){
		PyArrayObject* labels_arr =
			reinterpret_cast<PyArrayObject*>(labels_obj.ptr());
		CheckContiguousArray(labels_arr, "labels array", 1, 1, md_layer->label());
		if (PyArray_DIMS(data_arr)[0] != PyArray_DIMS(labels_arr)[0]) {
			throw std::runtime_error("data and labels must have the same first"
				" dimension");
		}
...
/*
void Net_SetStd(Net<Dtype>* net, string layer_name, bp::object data_obj, int nclass = 1) {
	// check that this network has an input MemoryDataLayer
	int idx = net->find_id_by_name(layer_name);
	shared_ptr<DiversityLossLayer<Dtype> > md_layer =
		boost::dynamic_pointer_cast<DiversityLossLayer<Dtype>>(net->layer_by_name(layer_name));//(net->layers()[idx]);
	PyArrayObject* data_arr = reinterpret_cast<PyArrayObject*>(data_obj.ptr());
	md_layer->set_std(static_cast<Dtype*>(PyArray_DATA(data_arr)), nclass);
}
void Net_SetSwitch(Net<Dtype>* net, string layer_name, int index = 0) {
	// check that this network has an input MemoryDataLayer
	int idx = net->find_id_by_name(layer_name);
	shared_ptr<SwitchLayer<Dtype> > md_layer =
		boost::dynamic_pointer_cast<SwitchLayer<Dtype>>(net->layer_by_name(layer_name));//(net->layers()[idx]);
	md_layer->select(index);
}
void Net_SetDirection(Net<Dtype>* net, string layer_name, Dtype direction = 1) {
	// check that this network has an input MemoryDataLayer
	int idx = net->find_id_by_name(layer_name);
	shared_ptr<LinearMeanLossLayer<Dtype> > md_layer =
		boost::dynamic_pointer_cast<LinearMeanLossLayer<Dtype>>(net->layer_by_name(layer_name));//(net->layers()[idx]);
	md_layer->set_direction(direction);
}
void Net_SetMean(Net<Dtype>* net, string layer_name, bp::object data_obj, int nclass = 1) {
	// check that this network has an input MemoryDataLayer
	int idx = net->find_id_by_name(layer_name);
	shared_ptr<MeanLossLayer<Dtype> > md_layer =
		boost::dynamic_pointer_cast<MeanLossLayer<Dtype>>(net->layer_by_name(layer_name));//(net->layers()[idx]);
	PyArrayObject* data_arr = reinterpret_cast<PyArrayObject*>(data_obj.ptr());
	md_layer->set_mean(static_cast<Dtype*>(PyArray_DATA(data_arr)), nclass);
}
*/
...
BOOST_PYTHON_MODULE(_caffe) {
  ...
  bp::class_<Net<Dtype>, shared_ptr<Net<Dtype> >, boost::noncopyable >("Net",
	...
	.def("set_input_arrays_with_name", &Net_SetInputArrays_with_name,
		bp::with_custodian_and_ward<1, 2, bp::with_custodian_and_ward<1, 3, bp::with_custodian_and_ward<1, 4> > >())
	//.def("set_loss_std", &Net_SetStd,
	//	bp::with_custodian_and_ward<1, 2, bp::with_custodian_and_ward<1, 3, bp::with_custodian_and_ward<1, 4> > >())
	//.def("set_loss_mean", &Net_SetMean,
	//	bp::with_custodian_and_ward<1, 2, bp::with_custodian_and_ward<1, 3, bp::with_custodian_and_ward<1, 4> > >())
	.def("select_net_with_idx", &Net<Dtype>::select_net_with_index)
	.def("clip_net_with_idx", &Net<Dtype>::select_weight_constrain_net)
	.def("clip_diffs",&Net<Dtype>::clip_diffs)
	.def("lock_net_with_idx", &Net<Dtype>::lock_net)
	//.def("set_mean_loss_direction",&Net_SetDirection )
	//.def("select_switch_with_name", &Net_SetSwitch,
	//	bp::with_custodian_and_ward<1, 2, bp::with_custodian_and_ward<1, 3> >())
	.def("set_loss_by_name",&Net<Dtype>::set_loss_weight_by_name)
    .def("save", &Net_Save)
  ...
  bp::class_<Solver<Dtype>, shared_ptr<Solver<Dtype> >, boost::noncopyable>(
	...
	.def("stepc", &Solver<Dtype>::Stepc)
    .def("restore", &Solver<Dtype>::Restore)
	.def("restart", &Solver<Dtype>::Restart)
	.def("reinit",&Solver<Dtype>::Reinit)
	.def("weight_decay",&Solver<Dtype>::set_weight_decay)
	.def("prestep",&Solver<Dtype>::Prestep)
	.def("applystep",&Solver<Dtype>::Applystep)
  ...
  //bp::class_<AdaBoundSolver<Dtype>, bp::bases<Solver<Dtype> >,
 //	  shared_ptr<AdaBoundSolver<Dtype> >, boost::noncopyable>(
//		  "AdaBoundSolver", bp::init<string>());
