...
void EltwiseLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (op_ == EltwiseParameter_EltwiseOp_DIV){
...
void EltwiseLayer<Dtype>::Forward_cpu(
  ...
  case EltwiseParameter_EltwiseOp_DIV:
	  caffe_copy(count, bottom[1]->cpu_data(), tmp_.mutable_cpu_data());
	  caffe_add_scalar(count, Dtype(0.000000001), tmp_.mutable_cpu_data());
	  caffe_div(count, ones_.cpu_data(), tmp_.cpu_data(), tmp_.mutable_cpu_data());
	  caffe_mul(count, bottom[0]->cpu_data(), tmp_.cpu_data(), top_data);
	break;
...
void EltwiseLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    if (propagate_down[i]) {
	  ...
	  case EltwiseParameter_EltwiseOp_DIV:
		  if (i == 0){
			  caffe_mul(count, tmp_.cpu_data(), top_diff, bottom_diff);
		  }
		  else{
			  caffe_cpu_axpby(count, Dtype(-1.0), top_diff, Dtype(0), bottom_diff);
			  caffe_mul(count, tmp_.cpu_data(), bottom_diff, bottom_diff);
			  caffe_mul(count, top_data, bottom_diff, bottom_diff);
		  }
	    break;
      case EltwiseParameter_EltwiseOp_SUM:
