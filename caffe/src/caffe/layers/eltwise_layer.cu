...
void EltwiseLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  case EltwiseParameter_EltwiseOp_DIV:
	  caffe_gpu_memcpy(count*sizeof(Dtype), bottom[1]->gpu_data(), tmp_.mutable_gpu_data());
	  caffe_gpu_add_scalar(count, Dtype(0.000000001), tmp_.mutable_gpu_data());
	  caffe_gpu_div(count, ones_.gpu_data(), tmp_.gpu_data(), tmp_.mutable_gpu_data());
	  caffe_gpu_mul(count, bottom[0]->gpu_data(), tmp_.gpu_data(), top_data);
	  break;
...
void EltwiseLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    if (propagate_down[i]) {
	  ...
	  case EltwiseParameter_EltwiseOp_DIV:
		  if (i == 0){
			  caffe_gpu_mul(count, tmp_.gpu_data(), top_diff, bottom_diff);
		  }
		  else{
			  caffe_gpu_axpby(count, Dtype(-1.0), top_diff, Dtype(0), bottom_diff);
			  caffe_gpu_mul(count, tmp_.gpu_data(), bottom_diff, bottom_diff);
			  caffe_gpu_mul(count, top_data, bottom_diff, bottom_diff);
		  }
		  break;
      case EltwiseParameter_EltwiseOp_SUM:
