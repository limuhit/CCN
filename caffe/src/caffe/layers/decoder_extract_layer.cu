...
namespace caffe {
	...
	__global__ void decoder_extract_gpu_kernel(const int num, const Dtype * const input,
	...
	template <typename Dtype>
	void DecoderExtractLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (label_) {
			...
			decoder_extract_gpu_kernel<Dtype> << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >
				(count, bottom_data, index, top_data, start_idx_[st], len_idx, height_, width_, 1, channel_, psum);
		}
		else {
			if (psum == 0) {
				...
				int len_idx = start_idx_[end] - start_idx_[st];
				int count = len_idx*num_ * 1;
				//LOG(INFO) << st << ":" << end<<":"<<len_idx<<":"<<count<<":"<<start_idx_[st]<<":"<<top[0]->count();
				decoder_extract_gpu_kernel<Dtype> << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >
					(count, bottom_data, index, top_data, start_idx_[st], len_idx, height_, width_, 1, channel_, psum);
			}
		}
	...
	template <typename Dtype>
	void DecoderExtractLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {

	}

	INSTANTIATE_LAYER_GPU_FUNCS(DecoderExtractLayer);
